 ...
 * @param length
 */
SitePercolationBallisticDeposition_v4::SitePercolationBallisticDeposition_v4(value_type length, bool periodicity)
        : SitePercolation_ps_v11(length, periodicity )
{

    std::cout << "Constructing SitePercolationBallisticDeposition_v4 object : line " << __LINE__ << endl;
 ...
 *
 */
void SitePercolationBallisticDeposition_v4::reset() {
    SitePercolation_ps_v11::reset();
    _search_position = 0;
}
 ...
 */
void SitePercolationBallisticDeposition_v4::initialize_indices() {
    cout << "empty : " << __LINE__ << endl;
}
 ...
 * @return
 */
Index SitePercolationBallisticDeposition_v4::select_site(vector<Index> &sites, vector<BondIndex> &bonds) {
    // randomly choose a site
    value_type r = std::rand() % (indices.size());

    Index current_site = _lattice.translate1DToSite(site_indices[indices[r]]);
 ...
 * @return
 */
Index SitePercolationBallisticDeposition_v4::select_site_upto_1nn_v2(
    ...
    Index current_site = _lattice.translate1DToSite(site_indices[index]);
//    cout << "current site " << current_site << endl;
    // find the bonds for this site
 ...
 * @return
 */
Index SitePercolationBallisticDeposition_v4::select_site_upto_2nn_v2(
        vector<Index> &sites, vector<BondIndex> &bonds
){
// randomly choose a site
    value_type  r = _search_position + (_random() % (randomized_index.size() - _search_position));
    value_type index = randomized_index[r];
    Index central_site = _lattice.translate1DToSite(site_indices[index]);
    Index selected_site{};
//    cout << "current site " << current_site << endl;
    // find the bonds for this site
...
/********************************************************
 * SitePercolationBallisticDeposition_v4
 ...
 * @return
 */
bool SitePercolationBallisticDeposition_v4::occupy() {
 ...
 */
value_type SitePercolationBallisticDeposition_v4::placeSite_1nn_v2() {
...
//    _last_placed_site = select_site_upto_1nn(sites, bonds);
    ...
    return placeSite(_lattice.translateSiteTo1D(_last_placed_site), sites, bonds);
}
 ...
 * @return
 */
value_type SitePercolationBallisticDeposition_v4::placeSite_2nn_v1() {
    vector<BondIndex> bonds;
    vector<Index>     sites;
