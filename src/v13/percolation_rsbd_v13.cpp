...
//# print("rnd ", rnd, " self.current_idx ", self.current_idx)
                // cout<<"{"; for(auto a: site_ids_indices){cout << a << ",";} cout<<"}" << endl;
...
P_STATUS SitePercolationL1_v13_Ptilda::select_site() {
//    cout << "Entry SitePercolationL1_v13_Ptilda::select_site " << current_idx << endl;
...
//        exit(-1);
        ...
        // cout << " selected " << central2 << endl;
        if (lattice_ref.get_site_by_id(central2).is_occupied()){
            count_Ptilda[occupied_site_count] += 1;
            lattice_ref.get_site_by_id(central_X).reduce_1st_nn();
            if (lattice_ref.get_site_by_id(central_X).is_removable(1)){
//# print("is_removable")
//# print("self.site_ids_indices before ", self.site_ids_indices)
...
SitePercolationL1_v13_Ptilda::SitePercolationL1_v13_Ptilda(int length, value_type seed, bool generate_seed):SitePercolationL1_v13(length, seed, generate_seed) {
//    setRandomState(seed, generate_seed);
    cout << "class : SitePercolationL1_v13_Ptilda" << endl;
    count_Ptilda.resize(length*length + 2); // extra 2 for safety
}
