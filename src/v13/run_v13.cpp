...
/**
 * @brief 2nd largest cluster information is written in file
 * 
 * @param length 
 * @param ensemble_size 
 */
...
//    percolation.init();
    ...
    std::vector<double> order_wraping(percolation.maxIterationLimit()),
            order_largest(percolation.maxIterationLimit());
    std::vector<double> largest_cluster_1st(percolation.maxIterationLimit());
    std::vector<double> largest_cluster_2nd(percolation.maxIterationLimit());
    std::vector<double> largest_cluster_3rd(percolation.maxIterationLimit());
    vector<double> tmp;
...
//        percolation.viewCluster(1);
    ...
    //    cout << "order " << endl;
        ...
        tmp = percolation.get_1st_largest_cluster_array();
        plus_equal(largest_cluster_1st, tmp);

        tmp = percolation.get_2nd_largest_cluster_array();
        plus_equal(largest_cluster_2nd, tmp);

        tmp = percolation.get_3rd_largest_cluster_array();
        plus_equal(largest_cluster_3rd, tmp);
...
//    fout_jump.close();
    ...
    std::string extension = "_L" + std::to_string(length) + '_' + tm + ".txt";
    std::string filename_entropy_order_parameter = percolation.get_signature() + "_entropy-order-cluster" + extension;


    std::ofstream fout(filename_entropy_order_parameter);
    fout << '#' << header_info.str() << std::endl;
    fout << "#<p>,<H(p,L)>,<P1(p,L)>,<P2(p,L)>,largest_cluster,2nd largest cluster,3rd largest cluster" << std::endl;
    fout << "#p = occupation probability" << std::endl;
    fout << "#H(p,L) = Entropy = sum( - u_i * log(u_i))" << std::endl;
    fout << "#P1(p,L) = Order parameter = (number of bonds in largest cluster) / (total number of bonds)" << std::endl;
    fout << "#P2(p,L) = Order parameter = (number of bonds in spanning or wrapping cluster) / (total number of bonds)" << std::endl;
    ...
    for(size_t i{}; i < percolation.maxIterationLimit(); ++i){
        ...
        fout << largest_cluster_1st[i] / double(ensemble_size) << delimiter;
        fout << largest_cluster_2nd[i] / double(ensemble_size) << delimiter;
        fout << largest_cluster_3rd[i] / double(ensemble_size) ;
        fout << std::endl;
    }
    fout.close();
}
...
void run_v13_rsbd_L2(int length, int ensemble_size){
    ...
    SitePercolationL2_v13 percolation(length, 0, true);
    percolation.setRandomState(0, true);
//    percolation.init();
